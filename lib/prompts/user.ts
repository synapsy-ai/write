export const userPrompts: {
  ...
  en: {
    ...
    ph_analysis_intro:
      "Write the introduction of the text analysis using the following structure: introduction, presentation of the text's theme, problematic (on the one hand, on the other, therefore), presentation of the author's thesis, announcement of the text's plan, using the author's arguments for each part. Text: [[PROMPT]]",
    ph_analysis_conclusion:
      "Write the conclusion of the text analysis, recall the most the most important elements of the development, clearly explaining how the development showed how the author demonstrated his thesis. Text: [[PROMPT]]",
    ph_analysis_basic:
  ...
  fr: {
    ...
    ph_analysis_intro:
      "Rédige l'introduction de l'explication de texte en utilisant la structure suivante : amorce, présentation du thème du texte, problématique (d'une part, d'autre part, donc), présentation de la thèse de l'auteur, annonce du plan du texte, en utilisant les arguments de l'auteur pour chaque partie. Texte : [[PROMPT]]",
    ph_analysis_conclusion:
      "Rédige la conclusion de l'explication de texte, rappelant les éléments les plus importants du développement, expliquant clairement comment le développement a montré comment l'auteur a démontré sa thèse. Texte : [[PROMPT]]",
    ph_analysis_basic:
  ...
  es: {
    ...
    es_ph_analysis_intro:
      "Escribe la introducción del análisis del texto utilizando la siguiente estructura: gancho, presentación del tema del texto, planteamiento del problema (por una parte, por otra parte, por tanto), presentación de la tesis del autor, anuncio del esquema del texto, utilizando los argumentos del autor para cada parte. Texto: [[PROMPT]]",
    es_ph_analysis_conclusion:
      "Escribe la conclusión del análisis del texto, recordando los elementos más importantes del desarrollo, explicando claramente cómo el desarrollo ha mostrado cómo el autor ha demostrado su tesis. Texto: [[PROMPT]]",
    es_ph_analysis_basic:
    ...
    es_ph_analysis_dev:
      "Escribe el análisis del texto utilizando frases, no una lista. La explicación es lineal, es decir, la disertación debe seguir el orden del texto. Cada parte corresponde a una parte del extracto, y dentro de la parte, las diversas explicaciones particulares (de términos, frases en sí mismas) se siguen como en el texto. Comienza con lo más general y termina con lo más específico: - Para cada parte, comienza dando la idea principal. - Para cada frase, da su función (premisa, explicación, ejemplo...) y su contenido expresado con tus propias palabras. Puede seguir una explicación si la frase es muy compleja. - Explica los términos filosóficos de la frase, dando definiciones, y explica cualquier significado implícito o referencia subyacente (puedes mencionar a otros autores). - Termina con un ejemplo concreto que ilustre la idea del autor. Texto: [[PROMPT]]",
    es_ph_analysis_outline:
